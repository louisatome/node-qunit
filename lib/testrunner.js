...
options = exports.options = {
    ...
    // if true, add report for each test of a module in the callback parameter
...
exports.run = function(files, callback) {
    ...
    files.forEach(function(file) {
        ...
        runOne(opts, function(err, stat) {
            if (err) {
                return callback(err, log.stats(options.completeReport));
            }
            ...
            if (filesCount >= files.length) {
                ...
                callback(null, log.stats(options.completeReport));
            }
        });
    });
};
